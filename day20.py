...
real ="""p=<2366,784,-597>, v=<-12,-41,50>, a=<-5,1,-2>
...
def prob1(input): 
    ...
    for i, line in enumerate(lines): 
        row = line.split(">")
        p = [int(x) for x in row[0][3:].split(',')]
        v = [int(x) for x in row[1][5:].split(',')]
        a = [int(x) for x in row[2][5:].split(',')]

        pos.append(p)
        vel.append(v)
        acc.append(a)

        a_mag = (a[0]**2 + a[1]**2 + a[2]**2)**.5
        if a_mag < a_max:
            a_max = a_mag
            i_a_max = i
