...
def parse_input(input): 
    ...
    for line in lines: 
        ...
        if not row: 
            break
        # print(row)
        data[row[0]] = {'weight': int(row[1][1:-1])}
        if len(row) > 2: 
            children = [c.strip().strip(',') for c in row[3:]]
            data[row[0]]['children'] = children

    return data


def get_node(s_name, tree):
    if tree is None: 
        return None 
    if s_name in tree: 
        return tree[s_name]

    for n_name, meta in tree.items(): 
        node = get_node(s_name, meta)
        if node is not None: 
            return node
    return None

def get_pnode(s_name, tree):
    if tree is None: 
        return None 
    if s_name in tree: 
        return tree
    ...
    for name, meta in data.items():
        ...
        if 'children' in meta: 
            for child in meta['children']: 
                ...
                child_node = get_node(child, tree) or {}
                node[child] = child_node
                p_node = get_pnode(child, tree)
                if p_node: 
                    del(p_node[child])
    return tree, data


def get_weight(s_name, tree, data): 

    tot = data[s_name]['weight']
    node = get_node(s_name, tree)
    # print(s_name, tot)
    for child in node: 
        tot += get_weight(child, tree, data)

    return tot

def check_children(s_name, tree, data): 
    
    weights = []
    c_names = []
    
    node = get_node(s_name, tree) or {}
    for child in node:     
        weights.append(get_weight(child, node, data))
        c_names.append(child)
    ...
    if len(weight_set) == 2: 
        indices = [i for i, x in enumerate(weights) if x == weight_set[0]]
        if len(indices) == 1: 
            bad_child = c_names[indices[0]]
            delta_w = weight_set[1] - weight_set[0]          
        else: 
            indices = [i for i, x in enumerate(weights) if x == weight_set[1]]
            bad_child = c_names[indices[0]]
            delta_w = weight_set[0] - weight_set[1]          

    return bad_child, delta_w

def search_weight(tree, data): 
    for n_name, children in tree.items(): 
        bad_child, delta_w_child = check_children(n_name, tree, data)
        if bad_child is None: 
            continue
       
        bad_grandchild, delta_w_grandchild = check_children(bad_child, children, data)

        if bad_grandchild is None: 
            return data[bad_child]['weight'] + delta_w_child
        else: 
            return search_weight(children, data)
...
cntj (57)
    ...
    tree, data = make_tree(test1_input)

    print('test 2 get_weight', get_weight('ugml', tree, data), 251)
    print('test 2 get_weight get_weight', get_weight('padx', tree, data), 243)
    print('test 2', get_weight('fwft', tree, data), 243)

    print('test 2', check_children('tknk', tree, data))
    print('test 2', check_children('ugml', tree, data))

    print('test 2', search_weight(tree, data))

    
    real_input = """xsddbi (61)
nqtowev (11)
xwohr (82)
flejt (36)
idwpug (54)
...
xufneyr (153)"""
    ...
    tree, data = make_tree(real_input)

    print('real 1', tree.keys())
    print('real 2', search_weight(tree, data))
